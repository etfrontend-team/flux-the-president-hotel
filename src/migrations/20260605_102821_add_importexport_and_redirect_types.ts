...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`exports\` (
  	...
  	\`collection_slug\` text DEFAULT 'redirects' NOT NULL,
  	\`where\` text DEFAULT '{}',
  ...
  await db.run(sql`ALTER TABLE \`redirects\` ADD \`type\` text DEFAULT '301' NOT NULL;`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`redirects\` DROP COLUMN \`type\`;`)
}
